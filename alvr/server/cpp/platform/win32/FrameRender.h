...
public:
	...
	bool Startup(FFRData ffrData, uint32_t renderWidth, uint32_t renderHeight);
	bool RenderFrame(ID3D11Texture2D *pTexture[][2], vr::VRTextureBounds_t bounds[][2], int layerCount, bool recentering, const std::string& message, const std::string& debugText, uint32_t renderWidth, uint32_t renderHeight);
