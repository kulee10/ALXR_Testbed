...
bool FrameRender::Startup(FFRData ffrData, uint32_t renderWidth, uint32_t renderHeight)
{
	...
	compositionTextureDesc.Width = renderWidth;
	compositionTextureDesc.Height = renderHeight;
	...
	if (enableColorCorrection) {
		...
		ColorCorrection colorCorrectionStruct = { (float)renderWidth, (float)renderHeight,
...
bool FrameRender::RenderFrame(ID3D11Texture2D *pTexture[][2], vr::VRTextureBounds_t bounds[][2], int layerCount, bool recentering, const std::string &message, const std::string& debugText, uint32_t renderWidth, uint32_t renderHeight)
{
	// Set render target
	m_pD3DRender->GetContext()->OMSetRenderTargets(1, m_pRenderTargetView.GetAddressOf(), m_pDepthStencilView.Get());
